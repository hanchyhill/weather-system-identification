...
import { cancelPrefetch, isServiceWorkerSupported, registerWeatherSW, prefetchLatest, setPrefetchOptions } from './utils/swClient'

createApp(App).use(createPinia()).mount('#app')

// 注册 Service Worker，并在“打开 / 标签页重新可见”时做一次 catch-up 预取：
// 始终只预取按生成延迟计算出的最新起报时次。
...
async function bootServiceWorker() {
  ...
  const kickPrefetch = () => {
    ...
    if (options.enabled) prefetchLatest(calLatestBaseTime(), options)
    else cancelPrefetch()
  }
