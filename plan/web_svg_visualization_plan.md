...
- 在 `src/draw/` 新增 Python SVG 生成脚本，使用本项目现有 `uv` 环境和 `xarray/matplotlib/cartopy/scipy`，生成透明背景 SVG 图层和前端可读取的分时次 `manifest.json`。
- 前端支持起报时次、预报时效、气压层、图层类型切换，并叠加槽线 JSON 数据绘制。

## Key Changes

- SVG 瓦片设计参考：
  - Python 生成逻辑参考 `H:\github\javascript\nwp_views\python\draw_img.py`，重点复用缩放等级、经纬度 bounds、透明 SVG 输出和文件命名思路。
  - 前端瓦片加载、可见瓦片判断、缓存和不同投影绘制逻辑参考 `H:\github\javascript\nwp_views\src\views\MapView.vue` 中的 `SvgTileManager`、`getRequiredTiles`、`drawSvgTiles`、`drawLambertTile`、`drawMercatorTile`。
  - IndexedDB 缓存参考 `H:\github\javascript\nwp_views\src\utils\indexedDBCache.js`。
  - 投影重采样 Worker 参考 `H:\github\javascript\nwp_views\src\workers\UnifiedProjectionWorkerManager.js` 和 `H:\github\javascript\nwp_views\src\workers\UnifiedProjectionWorker.js`。
...
- 前端应用：
  ...
  - 从 `data/products/{init_time}/manifest.json` 读取当前起报时次下可用的 `fc_hour`、`level`、`layer_type`，按选择加载对应 SVG。
...
- Python SVG 生成：
  - 新增 `src/draw/generate_svg_layers.py`，提供 CLI：
    ...
    ```bash
    uv run python src/draw/generate_svg_layers.py --init-time 2026062900 --fc-hours 000 003 --levels 200 500 850 925 950 --output data/products
    ```
  ...
  - 输出结构统一保存在 `data/products/` 下，每个 `init_time` 一个独立目录和一个独立 manifest：
...
- Python：
  ...
  - 用一个已有时次和少量层次生成 SVG，确认 `data/products/{init_time}/manifest.json` 结构完整、SVG 可打开且背景透明。
- 前端：
