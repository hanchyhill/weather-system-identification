...
===================
改进 @vis_web\src\components\MultiMapWorkspace.vue 的地图放大系数l跟瓦片层级的关系。
目前瓦片层级和放大系数继承 @vis_web\src\components\MapWorkspace.vue 
但是，在多图模式当中，每个子图的实际视觉大小跟MapWorkspace.vue 并不一致，一般比其更小，导致风向杆看起来非常密集。希望MultiMapWorkspace.vue有自己的自适应的地图放大系数跟瓦片层级的关系，在实际视觉上跟MapWorkspace.vue 保持一致。

=============
@现在，我让大学本科阶段的实习生帮我在 @vis_platform 前端平台审核AI撰写的幻灯片或者报告有没有问题（由 @src\new_weather_situation_agent.py 生成）。 把问题记录和反馈回来。 请帮我制定一套规范。帮我设计一个问题反馈表。 
...
天气影响与系统分析一致
======
把生成的SVG转换为jpg图像，并且叠加上天气系统。 脚本独立运行，基于目录变化检测。要求天气系统的可视化跟 前端 @vis_web/ 保持一致。
需要生成的图如下：

1. 500hPa天气形势图(500hPa风羽+高度场等值线+涡旋中心（保留中心涡度0.00006以上涡旋中心）+槽线(保留最小平均风速3m/s以上槽线，仅显示V上切变，V下切变))
2. 850hPa天气形势图(850hPa风羽+高度场等值线+涡旋中心+槽线(过滤最小平均风速3m/s，仅显示U左切变，U右切变)+涡旋轨迹
3. 925hPa天气形势图(925hPa风羽+高度场等值线+涡旋中心+槽线(过滤最小平均风速3m/s，仅显示U左切变，U右切变)+涡旋轨迹
4. 200hPa天气形势图(200hPa风羽+高度场等值线+涡旋中心（保留中心涡度0.00006以上涡旋中心）+槽线(保留最小平均风速3m/s以上槽线，仅显示V上切变，V下切变)+风速填色)

使用python脚本，根据上述要求，生成500hPa天气形势图，850hPa天气形势图，925hPa天气形势图。绘图风格接近气象地理科学绘图，需要叠加上色标等信息。

每种图片，分别绘制中国，华南，广东三个范围，三个范围使用的SVG层级参考 前端 @vis_web/ 保持一致。
